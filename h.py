...
# Define the surfaces and colors
ax.plot_surface(X, Y, 0, color=colors['Red'], alpha=0.5)
ax.plot_surface(X, Y, 1, color=colors['Green'], alpha=0.5)
ax.plot_surface(X, 0, Y, color=colors['Blue'], alpha=0.5)
ax.plot_surface(X, 1, Y, color=colors['Cyan'], alpha=0.5)
ax.plot_surface(X, Y, Y, color=colors['Magenta'], alpha=0.5)
ax.plot_surface(X, 1-Y, Y, color=colors['Yellow'], alpha=0.5)
ax.plot_surface(X, 1-Y, 0, color=colors['Black'], alpha=0.5)

# Add intersection points (colors)
intersection_points = {
    'Cyan': (0, 1, 1),
    'Black': (0, 0, 0),
    'Magenta': (1, 0, 1),
...
for name, (x, y, z) in intersection_points.items():
